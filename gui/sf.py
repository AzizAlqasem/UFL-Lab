...
def main():
    ...
    pe_I = pe_c1.number_input("Intensity [10^14 W/cm^2]", value=1.0)
    pe_wl = pe_c2.number_input("Wavelength (um)", value=0.8)
    st.write("Pondermotive Energy (eV)", pondermotive_energy(pe_I, pe_wl))

    st.markdown("""##### Keldysh Parameter""")
    kp_c1, kp_c2 = st.columns(2)
    kp_i_p = kp_c1.number_input("Ionization Potential (eV)", value=15.0)
    kp_u_p = kp_c2.number_input("Pondermotive Energy (eV)", value=5.0)
    st.write("Keldysh Parameter", keldysh_parameter(kp_i_p, kp_u_p))

    st.markdown("""##### Over the Barrier Intensity""")
    ob_c1, ob_c2 = st.columns(2)
    ob_i_p = ob_c1.number_input("Ionization Potential (eV)", value=15.0)
    ob_z = ob_c2.number_input("Charge of the ion (after ionization)", value=1.0)
